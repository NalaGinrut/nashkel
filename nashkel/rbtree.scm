...
(define (%insert-fixup-rec! head new fetch rotate1! rotate2!)
  (define p (tree-parent new))
  (define pp (rb-tree-grand-parent new))
  (define ppp (tree-parent pp))
  (define x (fetch pp))
  (define root (tree-root head))
  (cond
   ((and x (red? x))
    (black! p)
    (black! x)
    (red! pp)
    (%insert-fixup! head pp))
   (else
    (when (eq? new (fetch p))
      (rotate1! root p))
    (black! pp)
